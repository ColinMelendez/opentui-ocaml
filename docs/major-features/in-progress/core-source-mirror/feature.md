...
readiness, the quality of an individual module, or exact parity with every
reference option, host service, or asynchronous runtime mechanism.
...
| `renderer.ts` | `src/renderer.ml`; [`scheduler` feature record](../scheduler/feature.md); [`renderer-presentation` feature record](../renderer-presentation/feature.md) | Active translated owner | Renderer ownership, explicit frames, borrowed buffers, retained-tree rendering, scoped native opacity/scissor execution, hit-grid routing, focus, input dispatch, capability/palette response processing, transport-neutral renderer geometry, forced repaint invalidation, post-process callbacks, console ownership, terminal-session hooks, recoverable typed render-error reporting, global background color, and terminal cursor/mouse-pointer controls exist. Split-footer geometry is representable, but the reference split-footer replay, external-output capture, and scrollback surfaces are intentionally outside Core. Typed render-error handler failures preserve later delivery while programmer exceptions follow the owner runtime. Terminal setup/output is supplied by the Eio/Unix application session boundary; the renderer's Eio clock remains switch-owned while owner-domain/switch-checked frame scheduling and closure, post/console ordering, and scheduler consumers are implemented. |
| `types.ts` (`RenderContext`) | `src/render_context.ml`, with internal `src/renderer_events.ml`; [`renderer-presentation` feature record](../renderer-presentation/feature.md) | Active translated capability owner | Renderer-owned dimensions, copied terminal capabilities, typed owner-local event sources including render errors, lifecycle, focus, input, render-request state, and owner-scoped cursor/mouse-pointer presentation are composed through the context; resize invalidates cached pixel resolution. |
...
| `NativeSpanFeed.ts` | `src/native_span_feed.ml` and `opentui-raw/span_feed.ml` | Active translated reduction | Core exposes a synchronous copy-first/raw-ABI wrapper with typed options, copied spans, reservations, stats, draining, and ownership; it intentionally has no reference `onData`/`onError`, backpressure, or `idle` surface. Raw keeps the foreign feed lifetime and ABI token ownership. |
...
| `lib/clipboard.ts`, `lib/host-clipboard.internal.ts`, `lib/host-clipboard.native.ts` | `src/lib/clipboard.ml` | Active translated policy seam | The OCaml service is synchronous and injected over host and OSC52 terminal backends; it intentionally does not reproduce the reference asynchronous native host service or promise lifecycle. |
...
| `lib/render-geometry.ts` | `src/lib/render_geometry.ml` | Active portable geometry slice | Screen mode, footer, and clamped render dimensions are represented independently of terminal I/O; split-footer output/replay and scrollback remain application-owned. |
...
| `renderables/TextTable.ts` | `src/renderables/text_table.ml` | Active slice | Native cell views provide measurement/wrapping/drawing; retained table layout, typed per-column alignment, borders, updates, and alignment-aware pointer selection/copy are implemented. Styled cell chunks are represented by `Lib.Styled_text`; Markdown alignment propagation is an OCaml extension, not reference `TextTable` behavior. |
...
| `renderables/Code.ts` | `src/renderables/code.ml` | Active parser-backed slice | Code owns a native text-buffer renderable, injected parser client, optional owner-bound Eio execution for explicitly `Worker_safe` parsers, truthful pending/admission state, per-Code generation/coalescing and stale-result rejection, streaming initial-visibility and settled-buffer retention, a current-generation settlement promise, typed owner-domain callback contexts, one-shot cleanup through its exposed renderable identity, syntax-theme conversion, conceal/source-line mapping, selection, and explicit plain-text fallback. JavaScript workers and bundled WASM grammars are intentionally not claimed; callback promises are translated to direct-style functions and Eio promises. |
| `renderables/Diff.ts` | `src/renderables/diff.ml`, `src/renderables/diff_parser.ml` | Active parser-backed slice | Unified and split views compose Code and line-number children, validate unified hunks, preserve selection/copy, render parse errors, and synchronize split scrolling through the retained pointer route. Rebuilds replace changed side children and propagate the optional owner-bound Code background capability. The portable syntax/default line styling is narrower than the reference's full mutable Diff styling option surface. |
| `renderables/Markdown.ts`, `renderables/markdown-parser.ts` | `src/renderables/markdown.ml`, `src/renderables/markdown_parser.ml` | Active parser-backed slice | Typed Markdown blocks/inlines, stable-prefix content reconciliation, syntax styles, concealment, link detection, code blocks with propagated optional Code background capability and `draw_unstyled_text = not streaming`, tables, borders/layout, selection aggregation, and lifecycle cleanup are present. Parsed pipe-table alignment is projected into `Text_table` as an OCaml extension, not an upstream behavior. Markdown parsing remains synchronous, and the parser is an explicit subset rather than a claim of full `marked`/CommonMark parity. |
| `image.ts` | `src/image.ml`, `opentui-raw/image.ml`, `opentui-raw/image_stubs.c` | Active native-boundary slice | The callback-free Core owner synchronously admits encoded/RGBA/native values, snapshots mutable bytes, bounds Eio Path reads to 64 MiB, and separates structured read/decode/native failures while preserving exact raw decode errors. Metadata, materialization/copy, proportional or explicit resize, copy-transfer raw pixels, extraction, extension, transforms, and compositing are exposed; reference URL/Blob/Response and promise-based loading shapes, plus Node/Bun asset discovery, are not claimed. |
| `renderables/Image.ts` | `src/renderables/image.ml`, `src/owned_buffer.ml` | Active translated slice | Retained image sources resolve Kitty/Sixel/Blocks from explicit request and renderer capabilities, compute cell-aware fit/cover/fill geometry, pass source rectangles/pixel sizes through the clipping-aware native draw seam, optionally clear/draw through an owner-local off-screen buffer, and expose Empty/Loading/Ready/Failed with an optional owner-domain Eio Path-read lease, generation cancellation, immutable direct-style callbacks, failed-replacement preservation, source replacement, and one-shot cleanup through either identity. Reference URL/Blob/Response sources and `loadPromise` are intentionally omitted. |
...
| `lib/tree-sitter-styled-text.ts`, `lib/tree-sitter/{client,default-parsers,index,parsers-config,resolve-ft,types}.ts` | `src/tree_sitter_styled_text.ml`; `src/lib/tree_sitter_client.ml`, `tree_sitter_types.ml`, `tree_sitter_resolve_filetype.ml` | Active typed boundary | An injectable parser registry, pure resolved-parser runner, explicit `Owner_only`/`Worker_safe` contract, highlight metadata, UTF-8 codepoint ranges, conceal/injection styling, and fallback errors replace the reference JS/WASM worker boundary. Per-Code generation and latest-value admission live in `Renderables.Code`. |
...
scheduling remain Eio/application responsibilities. Split-footer geometry is
portable state only; external-output capture, replay, and scrollback surfaces
remain in that application/terminal-output layer. Renderer-owned timing now
