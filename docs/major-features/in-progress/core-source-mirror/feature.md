...
| `renderer.ts` | `src/renderer.ml`; [`scheduler` feature record](../scheduler/feature.md) | Active translated owner | Renderer ownership, explicit frames, borrowed buffers, retained-tree rendering, scoped native opacity/scissor execution, hit-grid routing, focus, input dispatch, capability/palette response processing, transport-neutral renderer geometry, forced repaint invalidation, post-process callbacks, console ownership, terminal-session hooks, and recoverable typed render-error reporting exist. Split-footer geometry is representable, but the reference split-footer replay, external-output capture, and scrollback surfaces are intentionally outside Core. Typed render-error handler failures preserve later delivery while programmer exceptions follow the owner runtime. Terminal setup/output is supplied by the Eio/Unix application session boundary; the renderer's Eio clock remains switch-owned while owner-domain/switch-checked frame scheduling and closure, post/console ordering, and scheduler consumers are implemented. |
| `types.ts` (`RenderContext`) | `src/render_context.ml`, with internal `src/renderer_events.ml` | Active translated capability owner | Renderer-owned dimensions, copied terminal capabilities, typed owner-local event sources including render errors, lifecycle, focus, input, and render-request state are composed through the context; resize invalidates cached pixel resolution. |
