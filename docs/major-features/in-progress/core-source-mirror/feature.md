...
| `renderables/Code.ts` | `src/renderables/code.ml` | Active parser-backed slice | Code owns a native text-buffer renderable, injected parser client, optional owner-bound Eio execution for explicitly `Worker_safe` parsers, truthful pending/admission state, per-Code generation/coalescing and stale-result rejection, one-shot cleanup through its exposed renderable identity, syntax-theme conversion, conceal/source-line mapping, selection, and explicit plain-text fallback. JavaScript workers and bundled WASM grammars are intentionally not claimed. |
