...
`Active` and `Translated` describe an implemented portable boundary, not
blanket TypeScript API parity. Runtime-specific options, asynchronous host
services, and performance mechanisms are called out in the row where the
correspondence intentionally stops.
...
| `renderer.ts` (`CliRenderer`) | [`renderable-core` feature record](major-features/in-progress/renderable-core/feature.md); [`scheduler` feature record](major-features/in-progress/scheduler/feature.md); [`renderer-presentation` feature record](major-features/in-progress/renderer-presentation/feature.md); [`renderer-output` feature record](major-features/in-progress/renderer-output/feature.md); `packages/opentui-core/src/renderer.ml` | Active translated core owner; Core keeps transport-neutral geometry, including split-footer dimensions, but does not expose the reference split-footer replay, external-output capture, or scrollback-surface APIs. Renderer-owned background color and terminal cursor/mouse-pointer presentation are implemented through the checked native owner. Renderer creation explicitly selects memory, stdout, or a feed-backed sink; Eio terminal setup/output remains the application boundary, with the owner-domain/switch-checked frame scheduler, serialized output flow, pre-render/teardown attachment seams, recoverable typed render-error reporting, post/console order, and scheduler consumers implemented. `Renderer.start_capability_detection` exposes the reference probe phase separately because the Eio harness owns alternate-screen and terminal-mode setup; the native probe ordering and response processing remain reference-backed. |
| `types.ts` (`RenderContext`) | `packages/opentui-core/src/render_context.ml`; internal `renderer_events.ml` is surfaced through `Render_context` and `Renderer`; [`renderer-presentation` feature record](major-features/in-progress/renderer-presentation/feature.md) | Active translated owner-local capability view; resize invalidates cached pixel resolution, render-error events preserve honest optional attribution, typed handler failures do not suppress later handlers, and owner-scoped cursor/mouse-pointer presentation forwards to native |
...
| `NativeSpanFeed.ts` | `packages/opentui-core/src/native_span_feed.ml`; foreign allocation and span lifetime remain in `packages/opentui-raw/span_feed.ml` | Active translated reduction; Core exposes a synchronous copy-first/raw-ABI wrapper with copied payloads, reservations, draining, and explicit release, not the reference `onData`/`onError`, backpressure, or `idle` surface |
...
| `lib/clipboard.ts`, `lib/host-clipboard.internal.ts`, `lib/host-clipboard.native.ts` | `packages/opentui-core/src/lib/clipboard.ml` | Active translated synchronous clipboard policy/OSC52 seam with injected host/terminal backends; the reference asynchronous native host service and promise lifecycle are intentionally not reproduced |
...
| `lib/tree-sitter-styled-text.ts`, `tree-sitter/*` | `packages/opentui-core/src/tree_sitter_styled_text.ml`; `packages/opentui-core/src/lib/tree_sitter_client.ml`, `tree_sitter_types.ml`, `tree_sitter_resolve_filetype.ml`; `packages/opentui-core/src/platform/eio_runtime/background.ml`; [background feature record](major-features/in-progress/background/feature.md) | Active typed parser seam; `Code` runs explicitly `Worker_safe` parser snapshots through the owner-bound Eio executor with per-owner generations, while `Owner_only` parsers remain synchronous; no JS/WASM asset loader or bundled grammar parity is claimed |
...
| `renderables/ScrollBox.ts`, `ScrollBar.ts` | `packages/opentui-core/src/renderables/scroll_box.ml`, `scroll_bar.ml` | Active slice; composed viewport/content ownership, culling, scrolling, bars, sticky positioning, acceleration, renderer-clock arrow repetition, and guarded live-frame edge auto-scroll |
| `renderables/Select.ts`, `TabSelect.ts` | `packages/opentui-core/src/renderables/select.ml`, `tab_select.ml` | Active slice; directional navigation, wrapping, descriptions, indicators, tabs, typed selection events, and optional ASCII-font option labels for Select |
...
| `renderables/Code.ts` | `packages/opentui-core/src/renderables/code.ml` | Active translated slice; injectable owner-only or Eio-background parser, truthful pending/admission state, per-owner generation/coalescing, stale-result rejection, streaming visibility/settled-buffer retention, current-generation settlement promise, typed direct-style callback contexts, renderable-identity cleanup, styled chunks, conceal/source-line mapping, selection, and fallback |
| `renderables/Diff.ts` | `packages/opentui-core/src/renderables/diff.ml`, `diff_parser.ml` | Active translated slice; unified/split layout, validated hunk parsing, line-number composition, selection, error view, split scroll synchronization, and portable syntax/default line styling. The reference's full mutable Diff styling option surface is intentionally narrower in OCaml |
| `renderables/Markdown.ts`, `renderables/markdown-parser.ts` | `packages/opentui-core/src/renderables/markdown.ml`, `markdown_parser.ml` | Active translated slice; synchronous typed block/inline parser, stable-prefix updates, styled links, Code children with propagated background capability and `draw_unstyled_text = not streaming`, tables, borders, selection aggregation, and explicit parser injection. Applying parsed pipe-table alignment to `Text_table` is an OCaml extension, not an upstream behavior |
| `renderables/TextTable.ts` | `packages/opentui-core/src/renderables/text_table.ml` | Active slice; measured/wrapped grid layout, typed per-column alignment, borders, native cell views, updates, and pointer selection/copy. Markdown alignment propagation is an OCaml extension rather than reference `TextTable` behavior |
| `renderables/Image.ts` | `packages/opentui-core/src/renderables/image.ml`, `packages/opentui-core/src/owned_buffer.ml` | Active translated slice; capability-driven Kitty/Sixel/Blocks selection, cell-aware fit/cover/fill sizing, clipping/fallback, optional retained off-screen buffering, truthful load state, owner-domain Eio Path read lease with generation/cancellation checks, immutable direct-style load callbacks, failed-replacement preservation, source replacement, and one-shot lifecycle cleanup. Reference URL/Blob/Response sources and `loadPromise` are intentionally omitted |
...
| `renderables/composition/*` | `packages/opentui-core/src/renderables/composition/{vnode,v_renderable,constructs}.ml` | Translated composition descriptions and constructors, including Code and typed styled-text conveniences; instantiation mounts ordinary retained identities, with no dynamic proxy identity |
...
| `animation/*` | [animation feature record](major-features/in-progress/animation/feature.md); `packages/opentui-core/src/animation` | Active translated slice; typed deterministic timelines, structured faults, explicit engine ownership, renderer pre-render attachment, and live leases are implemented; custom easing and framework bindings remain deferred |
| `plugins/*` | [plugins feature record](major-features/in-progress/plugins/feature.md); `packages/opentui-core/src/plugin*.ml`, `slot*.ml` | Active translated slice; typed transactional hosts, slots, retained mounts, structured failures, and renderer-bounded teardown are implemented; framework adapters and dynamic loading remain deferred |
...
| `audio.ts`, `audio-stream/*` | [audio-stream feature record](major-features/in-progress/audio-stream/feature.md); `packages/opentui-core/src/audio_stream.ml` | Active lifecycle kernel; owner-domain encoded-source lifecycle, cancellation, retries, metadata, terminal events, and generation safety are implemented; demuxers, native ABI/decoding, and HTTP integration remain deferred |
| `image.ts` | `packages/opentui-core/src/image.ml`; typed native seam in `packages/opentui-raw/image.ml` and `image_stubs.c` | Active translated slice; synchronous owner-domain encoded/RGBA/native operations, bounded Eio Path reads, distinct read/decode/native errors with exact raw decode mapping, vendored decoder metadata, proportional/explicit resizing, copy-transfer raw pixels, transforms, and compositing. The reference URL/Blob/Response and promise-based loading shapes are intentionally absent |
