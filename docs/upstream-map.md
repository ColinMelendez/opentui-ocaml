...
| `renderer.ts` (`CliRenderer`) | [`renderable-core` feature record](major-features/in-progress/renderable-core/feature.md); [`scheduler` feature record](major-features/in-progress/scheduler/feature.md); [`renderer-presentation` feature record](major-features/in-progress/renderer-presentation/feature.md); `packages/opentui-core/src/renderer.ml` | Active translated core owner; Core keeps transport-neutral geometry, including split-footer dimensions, but does not expose the reference split-footer replay, external-output capture, or scrollback-surface APIs. Renderer-owned background color and terminal cursor/mouse-pointer presentation are implemented through the checked native owner. Terminal setup/output is the Eio application/terminal-output boundary; the Eio clock, owner-domain/switch-checked frame scheduler and closure, pre-render/teardown attachment seams, recoverable typed render-error reporting, post/console order, and scheduler consumers are implemented |
