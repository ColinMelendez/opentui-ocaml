...
| `renderables/Markdown.ts`, `renderables/markdown-parser.ts` | `packages/opentui-core/src/renderables/markdown.ml`, `markdown_parser.ml` | Active translated slice; synchronous typed block/inline parser, stable-prefix updates, styled links, Code children with propagated background capability and `draw_unstyled_text = not streaming`, aligned tables, borders, selection aggregation, and explicit parser injection |
| `renderables/TextTable.ts` | `packages/opentui-core/src/renderables/text_table.ml` | Active slice; measured/wrapped grid layout, typed per-column alignment, borders, native cell views, updates, and pointer selection/copy |
| `renderables/Image.ts` | `packages/opentui-core/src/renderables/image.ml`, `packages/opentui-core/src/owned_buffer.ml` | Active slice; capability-driven Kitty/Sixel/Blocks selection, cell-aware fit/cover/fill sizing, clipping/fallback, optional retained off-screen buffering, truthful load state, owner-domain Path read lease with generation/cancellation checks, immutable load callbacks, failed-replacement preservation, source replacement, and one-shot lifecycle cleanup |
