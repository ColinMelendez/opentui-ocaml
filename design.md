...
The current `Handle.t` module is only a package-wiring scaffold. Its integer
representation is not the final ABI contract. The exact width, invalid value,
generation behavior, and thread/domain restrictions must come from the pinned
Zig source and smoke tests.
...
| Native event sink | A C-callable callback receives borrowed name/data pointers and lengths synchronously. Event names are dynamic byte strings. | In the first path, callbacks may run only synchronously on the UI owner. The C/OCaml boundary copies the bytes into an owned event packet before the callback returns; it does not invoke arbitrary handlers or re-enter the renderer. Exceptions become an explicit callback/error status. Background callbacks require a separate OCaml runtime-lock and queue design. |
