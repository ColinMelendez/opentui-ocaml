...
- `opentui-core` retained scenes and renderers, Yoga layout, `Box`, and plain
  `Text`, with explicit flushing and teardown;
...
implicit application loop. `Scene.flush` remains an explicit caller-owned
presentation boundary, and `Platform.Eio_runtime.Dispatch` remains a
caller-run dispatch loop over the bounded event handoff.

The repository also does not yet provide a general-purpose ordinary event
channel module. `Lib.Event_queue` is the bounded terminal-input handoff; it is
not the observer-channel abstraction described by the event-system design.
...
scene, input, output, resize, and terminal-session building blocks. Its target
contract is:
...
- an optional Lwd bridge over the retained tree; and
