...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
  printf '%s\n' "usage: sh packages/opentui-core/bench/trace_eio.sh OUTPUT.fxt [default|warmed]" >&2
  exit 2
fi
...
if [ "$workload" = warmed ]; then
  exec eio-trace record -f "$trace_file" -- \
    ...
    "$repo_dir/_build/default/packages/opentui-core/bench/profile.exe" --workload warmed
else
  exec eio-trace record -f "$trace_file" -- \
    nix develop "$repo_dir#test" -c \
    "$repo_dir/_build/default/packages/opentui-core/bench/profile.exe"
fi
