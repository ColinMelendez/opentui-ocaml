...
module Frame : sig
  ...
  (** [Ok count] reports the number of output bytes written. On success, the
      ...
      returns [Error (Native.Error.Native Output_too_small)] rather than a short
      write. *)
  val write_resolved_chars :
    t ->
    output:bytes ->
    add_line_breaks:bool ->
    (int32, Native.Error.t) result
end

(** [create ~width ~height] creates a renderer with positive dimensions. *)
val create : width:int32 -> height:int32 -> (t, Native.Error.t) result

(** [resize renderer ...] resizes the renderer when no frame is open. *)
val resize : t -> width:int32 -> height:int32 -> (unit, Native.Error.t) result
...
val run_frame :
  t ->
  force:bool ->
  draw:(Frame.t -> (unit, Native.Error.t) result) ->
  (render_status, Native.Error.t) result
