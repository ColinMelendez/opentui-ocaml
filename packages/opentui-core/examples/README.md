...
- `split_mode_demo.ml` is a port of the reference
...
Run the split-mode demo:

```sh
nix develop -c dune exec ./packages/opentui-core/examples/split_mode_demo.exe
```

Split-mode keys: Ctrl+Enter sends the composer, Enter inserts a newline,
Ctrl+S toggles streaming, Ctrl+0 switches between split-footer and fullscreen,
Ctrl+R queues a transcript, Shift+Up/Down changes the footer height (Ctrl+Up/Down
