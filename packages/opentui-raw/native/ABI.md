...
| `yogaNodeStyleSetValue` | `yoga.zig` | `void pointer, u32, u32, u32, f32 -> void` | The current typed surface binds point-valued width, height, and padding. Padding uses the pinned `YogaValueKind.padding = 8`, `YogaEdge` values left/top/right/bottom = `0/1/2/3`, and `YogaUnit.point = 1`; the C facade validates finite non-negative values before calling the export. Measure callbacks, packed style values, and other native renderable configuration remain deferred. |
