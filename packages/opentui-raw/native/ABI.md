...
| `yogaNodeInsertChild` / `yogaNodeRemoveChild` / `yogaNodeGetChildCount` | `yoga.zig:359`, `yoga.zig:363`, `yoga.zig:367` | `void pointer, void pointer, u32 -> void`; `void pointer, void pointer -> void`; `const void pointer -> u32` | Children are created with the tree's config and inserted synchronously under a node from the same tree. Removal validates the direct parent, recursively frees the detached native subtree, and invalidates its abstract node tokens. Cross-tree parents are rejected by the facade. |
