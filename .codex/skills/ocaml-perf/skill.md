...
1. **Per-iteration allocation**: closures in loops, repeated string
   ...
   path.
2. **Blocking I/O or wait**: time spent in syscalls, locks, or
   suspended fibers rather than in OCaml code.
3. **Polymorphic overhead**: `compare`, `(=)`, generic array ops
   where typed alternatives exist.
4. **Poor locality**: pointer-chasing through lists/trees when arrays
   would be contiguous.
5. **Missed inlining**: abstractions that are zero-cost only if the
   compiler sees through them (especially cross-module with `-opaque`
   or without Flambda).
6. **Write barrier cost**: mutating major-heap records to point to
   minor-heap values.
